...
for i in tqdm(range(frame_lim)):
    ...
    for idx, out in enumerate(output_cpu):
        normalized_out = (out - out.min()) / (out.max() - out.min())
        frame_filename = f"frame_{i}_{idx}.png"  # Unique filename for each frame
        plt.imsave(frame_filename, np.transpose(normalized_out, (1, 2, 0)))
        frame_files.append(frame_filename)  # Append the filename, not the frame itself
...
clip = mpy.ImageSequenceClip(frame_files, fps=22050/frame_length)
